...
If you want to save an entire workspace, rather than just individual files, we can use the command **save.image()** to do this. This saves not only data frames but any saved objects: models, plots, functions, etc.
...
We can then save these datasets so we can easily load them into R at a later date to continue working on them. We will use these in future classes. Here, we save the workspace, which includes just waves 1 and 2 of the Understanding Society data.
