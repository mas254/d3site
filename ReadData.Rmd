...
```{r cache = TRUE, warning = FALSE, message = FALSE}
write.csv(UndSoc.4, "myData/UndSocA.csv")
```
...
This applies to every method of saving data from R we will cover here.

### Saving data as an Excel file

To save our data as an Excel file, we need to first install the *xlsx* package, and use the command **write.xlsx()**.

```{r cache = TRUE, warning = FALSE, message = FALSE}
library(xlsx)

write.xlsx(UndSoc.4, "myData/UndSocEx.xlsx", sheetName="Example")
```
...
```{r cache = TRUE, warning = FALSE, message = FALSE}
write_sav(SPSS, "myData/test.sav")
```
...
If you want ot save an entire workspace, rather than just individual files, we can use the command **save.image()** to do this. This saves not only data frames but any saved objects: models, plots, functions, etc.
...
## Reading in multiple waves of Understanding Society
...
UndSoc2ed <- fread("data/UKDA-6614-tab/tab/us_w1/a_indresp.tab",
...
We can then save these datasets so we can easily load them into R at a later date to continue working on them. We will use these in future classes.

```{r cache = TRUE, warning = FALSE, message = FALSE}
write.csv(UndSoc1ed, "myData/UndSocA.csv")

write.csv(UndSo2ed, "myData/UndSocA.csv")
```