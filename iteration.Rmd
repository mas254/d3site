...
Another type of loop you will come across, though not one we will be using to load the *Understanding Society* data, is the **while** loop. Simply put, the while loop will carry out a command while certain criteria are filled, and stop once these are broken (in our example, once x <! 36). Let's say, for example, we now wanted to see the square and cube of numbers from 21 to 35. We can write this in a **while** loop in the following way:
...
To load the *Understanding Society* data, let us first consider a very simple **for** loop.
